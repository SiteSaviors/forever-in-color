...
export const useEntitlementsState = () =>
  useFounderStore(
    (state) => ({
      ...
      hasPremiumAccess: state.entitlements?.hasPremiumAccess ?? false,
      premiumTokens: state.entitlements?.premiumTokens ?? null,
      freeMonthlyTokens: state.entitlements?.freeMonthlyTokens ?? null,
