...
const MorphingCanvasPreview = ({
  ...
  return <div className={`relative cursor-pointer group ${isSelected ? 'scale-105' : 'hover:scale-102'} transition-all duration-500`} onClick={onClick} role="button" tabIndex={0} aria-pressed={isSelected} aria-label={`Select ${orientation} orientation`} onKeyDown={e => {
    if (e.key === 'Enter' || e.key === ' ') {
      ...
      {isRecommended && <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 z-20">
      ...
      <div className={`transition-all duration-300 ${morphing ? 'scale-95 opacity-50' : 'scale-100 opacity-100'}`}>
        <AspectRatio ratio={getAspectRatio()} className="relative overflow-hidden rounded-xl">
          ...
          <PremiumMaterialTextures orientation={orientation} size={size} isSelected={isSelected} className="absolute inset-0" />

          {/* Canvas Frame with Glass Effect */}
          <div className="relative w-full h-full">
            <img src={canvasFrame} alt={`${orientation} canvas frame`} className="w-full h-full object-contain relative z-10" />
          ...
          {userImageUrl && <div className={`absolute overflow-hidden transition-all duration-500 group-hover:brightness-110 z-15 ${imageLoaded ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} style={{
          top: imagePosition.top,
          left: imagePosition.left,
          width: imagePosition.width,
          height: imagePosition.height
        }}>
              <img src={userImageUrl} alt="Canvas preview" className="w-full h-full object-cover transition-all duration-500 group-hover:scale-105" style={{
          ...
          <div className={`absolute inset-0 border-2 rounded-xl transition-all duration-500 ${isSelected ? 'border-gradient-to-r from-purple-500 to-pink-500 shadow-2xl shadow-purple-500/30' : 'border-transparent group-hover:border-purple-300/50'}`} />
      ...
      {isSelected && <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 animate-bounce">
