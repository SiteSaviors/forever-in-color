  ...
  return <div className="space-y-8">
      ...
      <div className="flex items-center justify-center gap-8 mb-8">
        <div className={`flex items-center gap-2 ${hasImage ? 'text-green-600' : 'text-gray-400'}`}>
          ...
          {hasImage && <Badge variant="secondary" className="bg-green-100 text-green-700 text-xs">
        ...
        <div className={`flex items-center gap-2 ${hasStyle ? 'text-green-600' : hasImage ? 'text-purple-600' : 'text-gray-400'}`}>
          ...
          {hasStyle && <Badge variant="secondary" className="bg-green-100 text-green-700 text-xs">
              ✓ {selectedStyle?.name}
            </Badge>}
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            {hasImage ? `Your photo is ready with ${currentOrientation} canvas orientation. Now choose your art style.` : 'Upload your photo and select your canvas orientation in one seamless step.'}
          </p>
        </div>
        
        {!hasImage && <PhotoUpload onImageUpload={handleImageUpload} initialImage={uploadedImage} />}
      </div>

      {/* Style Selection Section - Only show after image is uploaded */}
      {hasImage && <div className="space-y-6">
          <div className="text-center">
          ...
          <StyleSelector croppedImage={uploadedImage} selectedStyle={selectedStyle?.id || null} onStyleSelect={handleStyleSelect} onComplete={handleStyleComplete} />
        </div>}
    </div>;
};
