...
export const useStyleCardInteractions = ({
  ...
  const cssClasses = useMemo(() => {
    ...
    if (visualState.isSelected) {
      return `${base} ring-4 sm:ring-5 ring-cyan-400/70 shadow-3xl shadow-cyan-400/30 scale-[1.03] sm:scale-[1.05] -translate-y-2 sm:-translate-y-3 premium-pulse bg-gradient-to-br from-white to-cyan-50/40 backdrop-blur-xl`;
    }
    
    if (visualState.isHovered) {
      return `${base} shadow-3xl ring-3 ring-fuchsia-400/50 scale-[1.02] sm:scale-[1.03] -translate-y-1 sm:-translate-y-2 backdrop-blur-xl glow-pulse`;
    }
    
    return `${base} shadow-xl hover:shadow-3xl hover:ring-3 hover:ring-fuchsia-400/50 hover:scale-[1.02] sm:hover:scale-[1.03] hover:-translate-y-1 sm:hover:-translate-y-2 hover:backdrop-blur-xl hover:glow-pulse`;
  }, [visualState]);
