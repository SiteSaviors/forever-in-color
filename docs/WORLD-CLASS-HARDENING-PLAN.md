...
| **5. Studio Component Decomposition** | Reduce component complexity | Incrementally extract `StyleSidebar` and `CanvasPreviewPanel`; replace direct store calls with memoized selectors/hooks; ensure existing props (e.g., `checkoutNotice`) continue to function. **Status: ⏳ In Progress** | Visual QA desktop/mobile, orientation/responsive checks |
